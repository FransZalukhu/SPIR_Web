...
<div class="container mx-auto px-4 py-6 font-manrope">
    ...
    <div class="mb-4">
        ...
        <select id="status-filter" class="mt-1 block w-60 rounded-md border-gray-300 shadow-sm">
...
<div id="detail-modal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 hidden z-50">
    <div class="bg-white rounded-lg p-6 w-1/2">
        ...
        <div id="detail-content" class="text-sm text-gray-700 space-y-2"></div>
...
<div id="update-modal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 hidden z-50">
    <div class="bg-white rounded-lg p-6 w-1/3">
        ...
        <form id="update-form">
            ...
            <select id="update-status" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                <option value="pending">Pending</option>
                <option value="diverifikasi">Diverifikasi</option>
                <option value="diproses">Diproses</option>
                <option value="selesai">Selesai</option>
            ...
            <div class="text-right mt-4">
                <button type="button" onclick="closeModal('update-modal')"
                    class="px-4 py-2 bg-gray-400 hover:bg-gray-600 text-white rounded">Batal</button>
                <button type="submit"
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function renderReports(reports) {
            ...
            if (reports.length === 0) {
                reportBody.innerHTML =
                    `<tr><td colspan="5" class="px-6 py-4 text-center text-gray-500">Tidak ada laporan ditemukan.</td></tr>`;
                return;
            }
        ...
        window.showDetail = function(report) {
            ...
            <p><strong>Isi Laporan:</strong> ${report.description ?? '-'}</p>
            ...
            <p><strong>Lokasi:</strong> ${report.latitude}, ${report.longitude}</p>
            ${report.photo_url ? `<div class="mt-2"><strong>Foto:</strong><br><img src="${report.photo_url}" alt="Foto Laporan" class="mt-2 rounded shadow max-w-full h-auto"/></div>` : ''}
        `;
